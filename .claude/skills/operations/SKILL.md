...
| `git init` + `git remote add` + initial commit (local only) | Provide `ECOSYSTEM_TOKEN` (org-level, or per repo) |
| Patch `workspace.json` + `mcp-test/.claude-plugin/marketplace.json` | Add `ECOSYSTEM_TOKEN` as Actions secret |
...
> **For `python-lib`: only step 1 applies, and the repo is `Seretos/lib-python-{feature}`.** A lib never dispatches into the marketplace, so there is **no marketplace-dispatch PAT** — its publish steps use only the built-in `secrets.GITHUB_TOKEN`. (Only the consumer-notification step needs `ECOSYSTEM_TOKEN`, which is normally an org-level secret already.) Skip steps 2 and 3 entirely. Tell the user: "Create the empty repo, then tell me when it's done."

Tell the user to do these things off-Claude. Then ask explicitly: "Tell me when those are done."
...
2. Make sure the repo can see the **`ECOSYSTEM_TOKEN`** secret — the one secret name for every ecosystem token (marketplace dispatch, ticket filing, …), a classic PAT with `repo` + `project` scope. If it is set at organisation level and shared with the new repo, nothing to do; otherwise add it under `Settings → Secrets and variables → Actions → New repository secret`, named exactly `ECOSYSTEM_TOKEN`.
...
- `python-lib/` — pure Python source library (no binary, no MCP, no marketplace). Its `release.yml` is the **lib-release flow** (semver-validate → stamp `pyproject.toml` → tag `vX.Y.Z` → force-push `release/Nx` → GitHub Release → open downstream dependency-update tickets; no binary, no marketplace dispatch, no `ECOSYSTEM_TOKEN`). The release's final step + the standalone `ticket.yml` recovery workflow (`open-dep-ticket`) both call the local composite action `.github/actions/notify-consumers`, which files a `chore(deps): bump {{lib_name}} to vX.Y.Z` issue (release changelog embedded, placed in Backlog on board #2) in each repo listed in `release.yml`'s `consumers:` input, via a `ECOSYSTEM_TOKEN` PAT — one secret name across all libs, never per-lib or per-consumer, so it can later become an org-level secret (Issues: write + `project` scope on the consumers; `GITHUB_TOKEN` can't open cross-repo issues). Labels (`ai-generated,task`), changelog and board placement are best-effort — a label the consumer lacks is skipped with a warning, never created, never a failure. Ships empty by design — a fresh lib has no consumers, so the step logs a notice and files nothing until the agent adds an `owner/repo` line in the same change that makes a downstream repo first pin the lib. Ships no `.claude-plugin`/`.codex-plugin` manifests, no `.spec`, no `build.ps1`, no icon, no `description.md`. Reference implementation: `libs/lib-python-config/` (the notification logic is defined by the template's `notify-consumers` action, not by any lib's copy). Placeholders: `{{lib_name}}` / `{{package_name}}` / `{{description}}` / `{{author_name}}`.
