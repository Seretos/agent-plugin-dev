# Build the {{plugin_name}} MCP server into a single-file Windows .exe.
#
# Usage (from plugin root):
#   pwsh -File scripts/build.ps1
#   pwsh -File scripts/build.ps1 -Clean      # remove dist/ build/ first
#   pwsh -File scripts/build.ps1 -Package    # also produce dist/{{plugin_name}}-<ver>.zip
#
...
$exe = Join-Path $root "dist\{{short_name}}.exe"
if (-not (Test-Path $exe)) {
    Fail "Expected dist/{{short_name}}.exe not produced."
}
$exeSize = [math]::Round((Get-Item $exe).Length / 1MB, 1)
Write-Host "    dist/{{short_name}}.exe (${exeSize} MB)"

# 5. Copy into bin/ where plugin.json expects it.
# Retries the copy because Defender briefly locks freshly-emitted .exe files.
# If the lock turns out to be a running {{short_name}}.exe (i.e. the dev's own
# Claude Code session has the plugin loaded), surface that clearly.
Write-Step "Copying to bin/{{short_name}}.exe"
New-Item -ItemType Directory -Force -Path "bin" | Out-Null
$copied = $false
for ($i = 0; $i -lt 5; $i++) {
    try {
        Copy-Item -Force $exe "bin/{{short_name}}.exe" -ErrorAction Stop
        $copied = $true
        break
...
if (-not $copied) {
    ...
    if ($running.Count -gt 0) {
        ...
        Write-Host "    {{short_name}}.exe is still running (PID: $pids)." -ForegroundColor Yellow
    ...
    Fail "Could not copy dist/{{short_name}}.exe to bin/ -- file remained locked."
}
...
$proc = Start-Process -FilePath "bin\{{short_name}}.exe" `
...
if ($Package) {
    ...
    $zipName = "{{plugin_name}}-$version.zip"
...
Write-Step "Done."
Write-Host "bin/{{short_name}}.exe is ready. Plugin manifest already points at it."
